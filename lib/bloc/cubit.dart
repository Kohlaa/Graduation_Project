...
class GradCubit extends Cubit<GradStates> {
  ...
  void createParking(
    ...
    DioHelper.postData(
      ...
      print('create parking response: ${value.data.toString()}');
  ...
  void getChatByUser(dynamic chatUserId, context) {
  ...
  // Future<VideoResponse> fetchVideoUrl() async {
  //   emit(AddCarLoadingState());
  //   try {
  //     Response response = await dio!.get('https://parkdetector.onrender.com/');
  //     if (response.statusCode == 200) {
  //       emit(AddCarSuccessState());
  //       videoResponse = VideoResponse.fromJson(response.data);
  //       return VideoResponse.fromJson(response.data);
  //     } else {
  //       emit(AddCarErrorState('s'));
